...
class Network(object):
    ...
    def stochastic_gradient_descent(self, training_data, epochs, mini_batch_size, eta,
        ...
        validation_mb_accuracy = theano.function([i], self.layers[-1].accuracy(self.y), givens={
        ...
        for epoch in xrange(epochs):
            for mini_batch_index in xrange(num_training_batches):
                ...
                if (iteration+1) % num_training_batches == 0:
                    validation_accuracy = np.mean([validation_mb_accuracy(j) for j in xrange(num_validation_batches)])
