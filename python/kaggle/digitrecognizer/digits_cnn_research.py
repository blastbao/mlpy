...
def main(_):
    ...
    W_fc1 = functions.weight_variable([7*7*64, 1000])
    b_fc1 = functions.bias_variable([1000])
    h_pool2_flat = tf.reshape(h_pool2, [-1, 7*7*64])
    h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)
    keep_prob = tf.placeholder(tf.float32)
    h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)
    W_fc2 = functions.weight_variable([1000, 1000])
    b_fc2 = functions.bias_variable([1000])
    h_fc2 = tf.nn.relu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
    h_fc2_drop = tf.nn.dropout(h_fc2, keep_prob)
    W_fc3 = functions.weight_variable([1000, 10])
    b_fc3 = functions.bias_variable([10])
